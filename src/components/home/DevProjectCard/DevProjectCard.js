...
function CustomToggle({ children, eventKey }) {
  ...
  return (
    <button
      ...
      className={`g_card-slide dev-project--full-width ${
        cardSlide ? "g_card-slide-up" : ""
      }`}
    >
      {children}
    </button>
...
function DevProjectCard({ img, title, tech, deployLink, repoLink }) {
  ...
  styledTech.forEach((element) => {
    techStyler.push(
      <h5 key={uuidv4()} className="dev-project__tech ">
        {element}
      </h5>
    );
  });
  ...
  return (
    <div
      className="g_projects-container dev-project__min-width"
      ref={devCardsRef}
    >
      <Accordion>
        <Card>
          <div className="neumorphism force-width">
            <CustomToggle eventKey="0">
              <img
                src={img}
                alt={title}
                className="g_project-img force-width"
              />
            </CustomToggle>
          </div>
                ...
                <div className="dev-project__links">
                  {!deployLink ? null : (
                    <a
                      ...
                      className={
                        !repoLink
                          ? "dev-project__button dev-project__button--full"
                          : "dev-project__button dev-project__button--half"
                      }
                    >
                      <FaDesktop />
                      <h5>Ver Página</h5>
